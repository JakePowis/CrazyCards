...
export default function Table({ user, toggleView, setToggleView }) {
  ...
  useEffect(() => {
    ...
    // redirect to form if no user data enetered (income as proxy)
    if (user.income === "") {
      setToggleView("form");
    }
  ...
  return (
    <div>
      ...
      <div className="total">
        <div>
          Total Credit Selected: £
          {selectedCards.reduce((a, card) => a + card.credit, 0)}
        </div>
      </div>

      <div className="buttonContainer">
        <button className="editButton" onClick={() => setToggleView("form")}>
          Edit Details
        </button>

        <button onClick={() => setToggleView("apply")} disabled>
          Apply
        </button>
      </div>
    </div>
