...
test("presents Ollie (test user #1) with two cards", () => {
  ...
  expect(
    matchCards(testUserOne, testCards).reduce((a, cards) => a + cards.credit, 0)
  ).toBe(3300);
});
...
test("presents Elizabeth (test user #2) with three cards", () => {
  ...
  expect(
    matchCards(testUserTwo, testCards).reduce((a, cards) => a + cards.credit, 0)
  ).toBe(4500);
});
...
test("presents Trevor (test user #3) with three cards", () => {
  ...
  expect(
    matchCards(testUserThree, testCards).reduce(
      (a, cards) => a + cards.credit,
      0
    )
  ).toBe(300);
});
